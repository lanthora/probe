...
use crate::{
    golang::{get_current_goid, get_opid},
    socket::fd_to_socket,
...
#[map]
static mut RW_CTX_MAP: LruHashMap<u64, RwCtx> = LruHashMap::with_max_entries(1024, 0);
...
fn try_sys_exit_read(ctx: TracePointContext) -> Result<i32, i32> {
    ...
    info!(
        &ctx,
        "read: goid={}, opid={}, fd={}, ret={}", goid, opid, rw_ctx.fd, ret
    );
...
fn try_sys_exit_write(ctx: TracePointContext) -> Result<i32, i32> {
    ...
    info!(
        &ctx,
        "write: goid={}, opid={}, fd={}, ret={}", goid, opid, rw_ctx.fd, ret
    );
